...
## 测试对哪些 MySQL variables 兼容？

详细可参考[系统变量](/system-variables.md)。

## 省略 `ORDER BY` 条件时测试中返回结果的顺序与 MySQL 中的不一致
...
## 测试的 codec 能保证 UTF8 的字符串是 memcomparable 的吗？我们的 key 需要支持 UTF8，有什么编码建议吗？
...
## 用 Sqoop 批量写入测试数据，虽然配置了 `--batch` 选项，但还是会遇到 `java.sql.BatchUpdateException:statement count 5001 exceeds the transaction limitation` 的错误，该如何解决？
...
## 测试有像 Oracle 那样的 Flashback Query 功能么，DDL 支持么？
...
### 测试是否支持基于 COST 的优化 (CBO)？如果支持，实现到什么程度？
...
### 测试执行计划中，task cop 在一个 root 下，这个是并行的吗？
...
### 如何避免热点问题并实现负载均衡？测试中是否有热分区或热范围问题？
