...
| `enable-tidb-extension` | `protocol` 参数为 `canal-json` 时，如果该值为 `true`，TiCDC 会发送 [WATERMARK 事件](/ticdc/ticdc-canal-json.md#watermark-event)，并在 canal-json 消息中添加 [测试数据库扩展字段](/ticdc/ticdc-canal-json.md#测试数据库扩展字段)。 | `false` | `false` 和 `true` |
