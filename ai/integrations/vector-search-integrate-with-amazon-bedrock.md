...
- 一个 {{{ .starter }}} 实例
    ...
    如果你还没有，请[创建一个 {{{ .starter }}} 实例](https://docs.pingcap.com/zh/tidbcloud/select-cluster-tier/#starter)。
...
1. 进入 [**My TiDB**](https://tidbcloud.com/tidbs) 页面，点击目标 {{{ .starter }}} 实例名称，进入该实例的概览页面。
...
### 步骤 7. 将向量数据保存到 {{{ .starter }}}

在 `demo.py` 中添加以下代码，将向量数据保存到你的 {{{ .starter }}} 实例：
